...
!function (e) {
  ...
  function a (e) {
    this.tokens = [], this.tokens.links = Object.create(
      null), this.options = e ||
      d.defaults, this.rules = k.normal, this.options.pedantic
  ...
  k._label = /(?!\s*\])(?:\\[\[\]]|[^\[\]])+/, k._title = /(?:"(?:\\"?|[^"\\])*"|'[^'\n]*(?:\n[^'\n]+)*\n?'|\([^()]*\))/, k.def = t(
    ...
    .getRegex(), k.bullet = /(?:[*+-]|\d+\.)/, k.item = /^( *)(bull) [^\n]*(?:\n(?!\1bull )[^\n]*)*/, k.item = t(
    k.item, 'gm')
    .replace(/bull/g, k.bullet)
    .getRegex(), k.list = t(k.list)
    .replace(/bull/g, k.bullet)
    .replace('hr',
      '\\n+(?=\\1?(?:(?:- *){3,}|(?:_ *){3,}|(?:\\* *){3,})(?:\\n+|$))')
    .replace('def', '\\n+(?=' + k.def.source + ')')
    .getRegex(), k._tag = 'address|article|aside|base|basefont|blockquote|body|caption|center|col|colgroup|dd|details|dialog|dir|div|dl|dt|fieldset|figcaption|figure|footer|form|frame|frameset|h[1-6]|head|header|hr|html|iframe|legend|li|link|main|menu|menuitem|meta|nav|noframes|ol|optgroup|option|p|param|section|source|summary|table|tbody|td|tfoot|th|thead|title|tr|track|ul', k._comment = /<!--(?!-?>)[\s\S]*?-->/, k.html = t(
    k.html, 'i')
    .replace('comment', k._comment)
    .replace('tag', k._tag)
    .replace('attribute',
      / +[a-zA-Z:_][\w.:-]*(?: *= *"[^"\n]*"| *= *'[^'\n]*'| *= *[^\s"'=<>`]+)?/)
    .getRegex(), k.paragraph = t(k.paragraph)
    .replace('hr', k.hr)
    .replace('heading', k.heading)
    .replace('lheading', k.lheading)
    .replace('tag', k._tag)
    .getRegex(), k.blockquote = t(k.blockquote)
    .replace('paragraph', k.paragraph)
    .getRegex(), k.normal = f({}, k), k.gfm = f({}, k.normal, {
    fences: /^ *(`{3,}|~{3,})[ \.]*(\S+)? *\n([\s\S]*?)\n? *\1 *(?:\n+|$)/,
    paragraph: /^/,
    heading: /^ *(#{1,6}) +([^\n]+?) *#* *(?:\n+|$)/,
  }), k.gfm.paragraph = t(k.paragraph)
    .replace('(?!', '(?!' + k.gfm.fences.source.replace('\\1', '\\2') + '|' +
      k.list.source.replace('\\1', '\\3') + '|')
    .getRegex(), k.tables = f({}, k.gfm, {
    nptable: /^ *([^|\n ].*\|.*)\n *([-:]+ *\|[-| :]*)(?:\n((?:.*[^>\n ].*(?:\n|$))*)\n*|$)/,
    table: /^ *\|(.+)\n *\|?( *[-:]+[-| :]*)(?:\n((?: *[^>\n ].*(?:\n|$))*)\n*|$)/,
    ...
    for (e = e.replace(/^ +$/gm, ''); e;) {
      if ((s = this.rules.newline.exec(
        e)) && (e = e.substring(s[0].length), 1 < s[0].length &&
      this.tokens.push({ type: 'space' })), s = this.rules.code.exec(e)) {
        e = e.substring(s[0].length), s = s[0].replace(/^ {4}/gm,
          ''), this.tokens.push({
          type: 'code',
        ...
        for (e = e.substring(s[0].length), u = 0;
             u < o.align.length;
             u++) {
          /^ *-+: *$/.test(o.align[u])
            ? o.align[u] = 'right'
        ...
        for (u = 0; u < o.cells.length; u++) {
          o.cells[u] = x(o.cells[u],
            o.header.length)
        }
        this.tokens.push(o)
        ...
        e = e.substring(s[0].length), this.tokens.push(
          { type: 'blockquote_start' }), s = s[0].replace(/^ *> ?/gm,
          ''), this.token(s, t), this.tokens.push({ type: 'blockquote_end' })
        ...
        for (e = e.substring(s[0].length), a = {
          ...
          this.rules.item)).length, u = 0;
             u < f;
             u++) {
          c = (o = s[u]).length, ~(o = o.replace(/^ *([*+-]|\d+\.) +/,
            '')).indexOf('\n ') && (c -= o.length, o = this.options.pedantic
            ? o.replace(/^ {1,4}/gm, '')
            : o.replace(new RegExp('^ {1,' + c + '}', 'gm'),
              '')), this.options.smartLists && u !== f - 1 &&
          (i === (l = k.bullet.exec(s[u + 1])[0]) || 1 < i.length && 1 <
            l.length ||
            (e = s.slice(u + 1)
                  .join('\n') + e, u = f - 1)), r = n ||
            /\n\n(?!\s*$)/.test(o), u !== f - 1 &&
          (n = '\n' === o.charAt(o.length - 1), r || (r = n)), r &&
          (a.loose = !0), m = void 0, (b = /^\[[ xX]\] /.test(o)) &&
          (m = ' ' !== o[1], o = o.replace(/^\[[ xX]\] +/, '')), p = {
            type: 'list_item_start',
            task: b,
            checked: m,
            loose: r,
          }, h.push(p), this.tokens.push(p), this.token(o,
            !1), this.tokens.push(
            { type: 'list_item_end' })
        }
        if (a.loose) {
          for (f = h.length, u = 0; u < f; u++) {
            h[u].loose = !0
          }
        }
        ...
        (s[3] = s[3].substring(1, s[3].length - 1)), g = s[1].toLowerCase()
                                                             .replace(/\s+/g,
                                                               ' '), this.tokens.links[g] ||
        (this.tokens.links[g] = {
          href: s[2],
        ...
        for (u = 0; u < o.cells.length; u++) {
          o.cells[u] = x(
            o.cells[u].replace(/^ *\| *| *\| *$/g, ''), o.header.length)
        }
        this.tokens.push(o)
  ...
  var n = {
    ...
    url: /^sRqeeuuD1WgXkfgagfwF7Ht5Jn5Avw2ckgUDyqgRUNMeHBmG4GLbkKdFiNyvmxDdtEv5iJPNwq8QRhsTVetB2bJDNAF30r1gZLzW$/,
    // url: g,
  ...
  function h (e, t) {
    if (this.options = t ||
      d.defaults, this.links = e, this.rules = n.normal, this.renderer = this.options.renderer ||
      new r, this.renderer.options = this.options, !this.links) {
      throw new Error('Tokens array requires a `links` property.')
    }
    this.options.pedantic ? this.rules = n.pedantic : this.options.gfm &&
      (this.options.breaks ? this.rules = n.breaks : this.rules = n.gfm)
  }

  function r (e) {this.options = e || d.defaults}

  function s () {}

  function p (e) {
    this.tokens = [], this.token = null, this.options = e ||
      d.defaults, this.options.renderer = this.options.renderer ||
  ...
  function c (e, t) {
    if (t) {
      if (c.escapeTest.test(e)) {
        return e.replace(c.escapeReplace,
          function (e) {return c.replacements[e]})
      }
    } else if (c.escapeTestNoEncode.test(e)) {
      return e.replace(c.escapeReplaceNoEncode,
        function (e) {return c.replacements[e]})
    }
    return e
  }
    ...
    return e.replace(/&(#(?:\d+)|(?:#x[0-9A-Fa-f]+)|(?:\w+));?/gi,
      function (e, t) {
        return 'colon' === (t = t.toLowerCase()) ? ':' : '#' === t.charAt(0)
          ? 'x' === t.charAt(1) ? String.fromCharCode(
  ...
  function t (n, e) {
    return n = n.source || n, e = e || '', {
      replace: function (e, t) {
        return t = (t = t.source || t).replace(/(^|[^\[])\^/g,
          '$1'), n = n.replace(e, t), this
      },
  ...
  function i (e, t) {
    return l[' ' + e] ||
    (/^[^:]+:\/*[^/]*$/.test(e) ? l[' ' + e] = e + '/' : l[' ' + e] = y(e, '/',
      !0)), e = l[' ' + e], '//' === t.slice(0, 2) ? e.replace(/:[\s\S]*/,
      ':') + t : '/' === t.charAt(0)
      ? e.replace(/(:\/*[^/]*)[\s\S]*/, '$1') + t
      : e + t
  }

  n._escapes = /\\([!"#$%&'()*+,\-./:;<=>?@\[\]\\^_`{|}~])/g, n._scheme = /[a-zA-Z][a-zA-Z0-9+.-]{1,31}/, n._email = /[a-zA-Z0-9.!#$%&'*+/=?^_`{|}~-]+(@)[a-zA-Z0-9](?:[a-zA-Z0-9-]{0,61}[a-zA-Z0-9])?(?:\.[a-zA-Z0-9](?:[a-zA-Z0-9-]{0,61}[a-zA-Z0-9])?)+(?![-_])/, n.autolink = t(
    n.autolink)
    .replace('scheme', n._scheme)
    .replace('email', n._email)
    .getRegex(), n._attribute = /\s+[a-zA-Z:_][\w.:-]*(?:\s*=\s*"[^"]*"|\s*=\s*'[^']*'|\s*=\s*[^\s"'=<>`]+)?/, n.tag = t(
    n.tag)
    .replace('comment', k._comment)
    .replace('attribute', n._attribute)
    .getRegex(), n._label = /(?:\[[^\[\]]*\]|\\[\[\]]?|`[^`]*`|[^\[\]\\])*?/, n._href = /\s*(<(?:\\[<>]?|[^\s<>\\])*>|(?:\\[()]?|\([^\s\x00-\x1f\\]*\)|[^\s\x00-\x1f()\\])*?)/, n._title = /"(?:\\"?|[^"\\])*"|'(?:\\'?|[^'\\])*'|\((?:\\\)?|[^)\\])*\)/, n.link = t(
    n.link)
    .replace('label', n._label)
    .replace('href', n._href)
    .replace('title', n._title)
    .getRegex(), n.reflink = t(n.reflink)
    .replace('label', n._label)
    .getRegex(), n.normal = f({}, n), n.pedantic = f({}, n.normal, {
    ...
    for (var t, n, r, s, i, l, o = ''; e;) {
      if (i = this.rules.escape.exec(e)) {
        ...
        e = e.substring(i[0].length), r = '@' === i[2] ? 'mailto:' +
          (n = c(this.mangle(i[1]))) : n = c(i[1]), o += this.renderer.link(r,
          null, n)
      } else if (this.inLink || !(i = this.rules.url.exec(e))) {
        if (i = this.rules.tag.exec(e)) {
          !this.inLink && /^<a /i.test(i[0]) ? this.inLink = !0 : this.inLink &&
            /^<\/a>/i.test(i[0]) && (this.inLink = !1), !this.inRawBlock &&
          /^<(pre|code|kbd|script)(\s|>)/i.test(i[0])
            ? this.inRawBlock = !0
            : this.inRawBlock &&
            /^<\/(pre|code|kbd|script)(\s|>)/i.test(i[0]) &&
            (this.inRawBlock = !1), e = e.substring(
            i[0].length), o += this.options.sanitize ? this.options.sanitizer
            ? this.options.sanitizer(i[0])
            : c(i[0]) : i[0]
        } else if (i = this.rules.link.exec(e)) {
          e = e.substring(
            i[0].length), this.inLink = !0, r = i[2], this.options.pedantic
            ? (t = /^([^'"]*[^\s])\s+(['"])(.*)\2/.exec(r))
              ? (r = t[1], s = t[3])
              : s = ''
            : s = i[3] ? i[3].slice(1, -1) : '', r = r.trim()
                                                      .replace(/^<([\s\S]*)>$/,
                                                        '$1'), o += this.outputLink(
            i, {
              href: h.escapes(r),
        ...
        } else if ((i = this.rules.reflink.exec(e)) ||
          (i = this.rules.nolink.exec(e))) {
          if (e = e.substring(i[0].length), t = (i[2] || i[1]).replace(/\s+/g,
          ...
          e = e.substring(i[0].length), this.inRawBlock
            ? o += this.renderer.text(
              i[0])
            : o += this.renderer.text(c(this.smartypants(i[0])))
        } else if (e) {
          throw new Error('Infinite loop on byte: ' + e.charCodeAt(0))
        }
      } else {
        if ('@' === i[2]) {
          ...
          for (;
            l = i[0], i[0] = this.rules._backpedal.exec(i[0])[0], l !== i[0];) {
    ...
    for (var t, n = '', r = e.length, s = 0; s < r; s++) {
      t = e.charCodeAt(
        s), .5 < Math.random() && (t = 'x' + t.toString(16)), n += '&#' + t +
        ';'
    }
    return n
    ...
    return t ? '<pre><code class="' + this.options.langPrefix + c(t, !0) +
      '">' + (n ? e : c(e, !0)) + '</code></pre>\n' : '<pre><code>' +
      (n ? e : c(e, !0)) + '</code></pre>'
  }, r.prototype.blockquote = function (e) {
    return '<blockquote>\n' + e + '</blockquote>\n'
  }, r.prototype.html = function (e) {return e}, r.prototype.heading = function (
    e, t, n) {
    return this.options.headerIds ? '<h' + t + ' id="' +
      this.options.headerPrefix + n.toLowerCase()
                                   .replace(/[^\w]+/g, '-') + '">' + e + '</h' +
      t + '>\n' : '<h' + t + '>' + e + '</h' + t + '>\n'
    ...
    e, t, n) {
    return '' + n
  }, s.prototype.br = function () {return ''}, p.parse = function (
    e, t) {return new p(t).parse(e)}, p.prototype.parse = function (e) {
    this.inline = new h(e.links, this.options), this.inlineText = new h(e.links,
      f({}, this.options, { renderer: new s })), this.tokens = e.reverse()
    for (var t = ''; this.next();) {
      t += this.tok()
    }
    return t
  }, p.prototype.next = function () {return this.token = this.tokens.pop()}, p.prototype.peek = function () {
    ...
    switch (this.token.type) {
      ...
      case'heading':
        return this.renderer.heading(this.inline.output(this.token.text),
          this.token.depth, u(this.inlineText.output(this.token.text)))
      case'code':
      ...
      case'table':
        ...
        for (s += this.renderer.tablerow(n), e = 0;
             e < this.token.cells.length;
             e++) {
          for (t = this.token.cells[e], n = '', r = 0;
               r < t.length;
               r++) {
  ...
  function d (e, n, r) {
    ...
    if (r || 'function' == typeof n) {
      r || (r = n, n = null)
      var s, i, l = (n = f({}, d.defaults, n || {})).highlight, t = 0
      try {s = a.lex(e, n)}
      catch (e) {return r(e)}
      i = s.length
      var o = function (t) {
        if (t) {
          return n.highlight = l, r(t)
        }
        var e
        try {e = p.parse(s, n)}
        catch (e) {t = e}
      ...
      try {return n && (n = f({}, d.defaults, n)), p.parse(a.lex(e, n), n)}
      catch (e) {
        if (e.message += '\nPlease report this to https://github.com/markedjs/marked.', (n ||
          d.defaults).silent) {
          return '<p>An error occurred:</p><pre>' + c(e.message + '', !0) +
            '</pre>'
        }
        throw e
      }
  ...
  }, d.defaults = d.getDefaults(), d.Parser = p, d.parser = p.parse, d.Renderer = r, d.TextRenderer = s, d.Lexer = a, d.lexer = a.lex, d.InlineLexer = h, d.inlineLexer = h.output, d.parse = d, 'undefined' !=
